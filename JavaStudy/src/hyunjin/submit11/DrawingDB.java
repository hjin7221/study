...
public class DrawingDB {
	...
	public void registDrawing(Drawing draw) {
		for(int i = 0; i < drawingList.size(); i++) {
			if(draw.getPjt().equals(drawingList.get(i).getPjt()) && draw.getProduct().equals(drawingList.get(i).getProduct()) && draw.getDate().equals(drawingList.get(i).getDate()) ) {
	...
	public void searchDrawing(String keyword) {
		boolean c = false;
		for(int i = 0; i < drawingList.size(); i++) {
			if(drawingList.get(i).getPjt().toLowerCase().contains(keyword) || drawingList.get(i).getProduct().toLowerCase().contains(keyword)) {
				System.out.println(drawingList.get(i));
			}
			c = true;
		}
	...
	public void printDrawing(int no) {
		...
		for (int i =0; i<drawingList.size(); i++) {
			if(drawingList.get(i).getNo() == no) {
				System.out.println("선택하신 " + drawingList.get(i).getPjt()+"프로젝트 " + drawingList.get(i).getProduct() + " 제품 도면을 출력합니다.");
			}
			check = true;
		}
		if ( check = false) {
